...
while response != "N":
    ...
    preference = input()
    customer_preferences.append(preference)  # Add the customer preference to the list
...
def recommend_products(products, customer_tags):
    ...
    for product in products:
        ...
        if match_count > 0:  # Only include products with at least one match
            matches.append({
                'name': product['name'],
                'matches': match_count
            })
...
for rec in recommendations:
    print(f"Product: {rec['name']}, Matches: {rec['matches']}")
